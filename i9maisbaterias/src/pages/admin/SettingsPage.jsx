...
export function SettingsPage() {
    ...
    const [calcPrices, setCalcPrices] = useState(null);
    ...
    useEffect(() => {
        setIsLoading(true);
        Promise.all([
            fetch(`${API_URL}/api/admin/settings/contact-email`, { credentials: 'include' }).then(res => res.json()),
            fetch(`${API_URL}/api/impact-data`).then(res => res.json()),
            fetch(`${API_URL}/api/calculator-prices`).then(res => res.json())
        ]).then(([emailData, impactData, calcData]) => {
            setEmail(emailData.email);
            setImpactData(impactData);
            setCalcPrices(calcData);
        }).catch(err => {
            console.error("Falha ao carregar configurações", err);
            setEmailStatus("Erro ao carregar dados.");
        }).finally(() => {
            setIsLoading(false);
        });
    }, []);
    ...
    if (isLoading) {
        return (
            <>
                <header className="admin-header"><h1>Configurações Gerais</h1></header>
            ...
            <main className="admin-page-content">
                <div className="settings-grid">
                    <div className="admin-card">
                        <div className="admin-card-header"><h2>E-mail de Notificação</h2></div>
                    ...
                    {calcPrices && (
                        <div className="admin-card">
                            <div className="admin-card-header"><h2>Valores Base das Calculadoras</h2></div>
                            <div className="admin-card-body">
                                <form onSubmit={handleCalcPricesSave} className="admin-form">
                                    <p>Valores usados nos cálculos de ROI e Economia.</p>
                    ...
                    {impactData && (
                        <div className="admin-card full-width-card">
                            <div className="admin-card-header"><h2>Números do Dashboard de Impacto</h2></div>
                            <div className="admin-card-body">
                                <form onSubmit={handleImpactSave} className="admin-form">
                                    <p>Valores que aparecem na seção "Nosso Impacto em Números".</p>
